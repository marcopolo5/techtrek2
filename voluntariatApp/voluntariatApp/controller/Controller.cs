	...
	internal class Controller
	{
		...
		public bool login(string username, string password)
		{
			try
			{
				...
				// Găsește utilizatorul asociat CNP-ului din `loggedInEntity`
				if (this.loggedInEntity != null)
				{
					this.user = this.service.getUser(this.loggedInEntity.getId());
					return true;
				}
			}
			catch (Exception _)
		...
		public void eventRegistration(long idEvent, string reason)
		{

			// Apelare serviciu cu CNP-ul utilizatorului curent
			service.addSignup(loggedInEntity.getId(), idEvent, reason);
		}
